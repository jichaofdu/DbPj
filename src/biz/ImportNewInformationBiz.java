...
public class ImportNewInformationBiz {
	...
	public boolean insertSetOfNewUser(String path) throws IOException,
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (type == -1) {
				...
				SystemManager newSystemManager = new SystemManager(-1,
						password, name);
	...
	public void insertSetNewProject(String path) throws InvalidFormatException,
			IOException {
		InputStream inp = new FileInputStream(path);
		Workbook wb = WorkbookFactory.create(inp);
		Sheet sheet = wb.getSheetAt(0);
		...
		for (int i = 0; i < values.length; i++) {
			...
			Project tempProject = new Project(pjId, manager, pjName,
					pjDescription);
